...
func Test_service_InitiateRefund(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	setup := func() (
		...
		*stripeClient.MockClient,
		...
		return svc, ctx, mockStripeClient
	}

	t.Run("Valid refund with amount", func(t *testing.T) {
		svc, ctx, mockStripeClient := setup()
		req := &entities.StripeRefundRequest{
			PaymentIntentId: "pi_123",
			Body: &entities.StripeRefundRequestBody{
				Amount: "10.50",
			},
		}

		mockStripeClient.EXPECT().
			Refund(ctx, gomock.Any()).Return(&stripeEntities.RefundResponse{
			Id:     "re_123",
			Status: "succeeded",
		}, nil).Times(1)

		resp, err := svc.InitiateRefund(ctx, req)

		assert.NoError(t, err)
		assert.Equal(t, "re_123", resp.Id)
		assert.Equal(t, "succeeded", resp.Status)
	})

	t.Run("Valid refund without amount (full refund)", func(t *testing.T) {
		svc, ctx, mockStripeClient := setup()
		req := &entities.StripeRefundRequest{
			PaymentIntentId: "pi_123",
			Body: &entities.StripeRefundRequestBody{
	...
	t.Run("Invalid amount format", func(t *testing.T) {
		...
		_, err := svc.InitiateRefund(ctx, req)
